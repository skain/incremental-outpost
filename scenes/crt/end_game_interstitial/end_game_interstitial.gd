...
func run_interstitial() -> void:
	...
	conversion_message.text = conversion_message.text.replace("%points%", str(points_to_convert)).replace("%bucks_per%", str(SkillsManager.get_as_float(SkillTreeNode.AffectedStat.BUCKS_CONVERSION_RATE)))
